...
#if defined(NDEBUG)
# error "SincereCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc34f16f2f791d2423b6c0c650e3036eb28d3f9a384bd19af6270fdecaadb23b7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SincereCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SincereCoin Signed Message:\n";
...
{
    // SincereCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 200 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 1111111 * COIN;
            }

    // Subsidy is cut in half every 25000 blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SincereCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SincereCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1474490708, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "21 09 2016 evening";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e3bf99df188384111f4bf31642db2a41a2bf9f1bb33bcc697ff6488dfc0fec7c8c6a4720921a7e700ad0b9af21e85084a17ebf8f579fa73400cbf5e70dc12205") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1474490708;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1092966;
        ...
        assert(block.hashMerkleRoot == uint256("0xb83df9b110d2bc1fefebe0e243dac4b2db3b775e54c482aac9701b9b53fc87fe"));
...
unsigned char pchMessageStart[4] = { 0xba, 0x49, 0x02, 0x9c }; // SincereCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SincereCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SincereCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SincereCoinMiner(CWallet *pwallet)
{
    printf("SincereCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sincerecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SincereCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SincereCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SincereCoinMiner, pwallet));
}
